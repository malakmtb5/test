...
spacex_df = pd.read_csv("./jupyter/capstone/spacex_launch_dash.csv")
max_payload = spacex_df['Payload Mass (kg)'].max()
min_payload = spacex_df['Payload Mass (kg)'].min()

opts = [{'label': str(x) , 'value': str(x)}  for x in list(spacex_df['Launch Site'].unique())]
opts.append({'label': 'All Sites', 'value': 'ALL'})
#site_dropdown = spacex_df['Launch Site'][0]
